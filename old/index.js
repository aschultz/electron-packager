...
class Packager {
    ...
    skipHostPlatformSansSymlinkSupport(comboOpts) {
        common.info(
            `Cannot create symlinks (on Windows hosts, it requires admin privileges); skipping ${
                comboOpts.platform
            } platform`,
            this.opts.quiet
        );
    ...
    overwriteAndCreateApp(outDir, comboOpts, zipPath) {
        ...
        return fs.remove(outDir).then(() => this.createApp(comboOpts, zipPath));
    }

    extractElectronZip(comboOpts, zipPath, buildDir) {
        debug(`Extracting ${zipPath} to ${buildDir}`);
        return pify(extract)(zipPath, { dir: buildDir }).then(() =>
            hooks.promisifyHooks(this.opts.afterExtract, [
                buildDir,
    ...
    createApp(comboOpts, zipPath) {
        ...
        common.info(
            `Packaging app for platform ${comboOpts.platform} ${comboOpts.arch} using electron v${
                comboOpts.electronVersion
            }`,
            this.opts.quiet
        );
    ...
    packageForPlatformAndArch(downloadOpts) {
        return download.downloadElectronZip(downloadOpts).then((zipPath) => {
        ...
        .then(() =>
            Promise.all(
                download
                    ...
                    .map((downloadOpts) => packager.packageForPlatformAndArch(downloadOpts))
            )
        );
}
...
module.exports = function packager(opts) {
    ...
    return (
        getMetadataFromPackageJSON(platforms, opts, packageJSONDir)
            .then(() => {
